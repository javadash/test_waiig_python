...
class Evaluator:
    ...
    def _eval_statements(self, stmts : List[ast.Statement],
                      env: environment.Environment) -> monkey_object.MonkeyObject:
        result : monkey_object.MonkeyObject() = monkey_object.Null()
    ...
    def _eval_integer_infix_expression(self, operator: str, left: monkey_object.MonkeyObject,
                                       right: monkey_object.MonkeyObject) -> \
                                       monkey_object.MonkeyObject:
        # Called from _eval_infix_expression which lack type assertion. mypy
    ...
    def _eval_if_expression(self, expr: ast.IfExpression, env: environment.Environment) \
                                -> monkey_object.MonkeyObject:
    ...
    def _eval_program(self, stmts: List[ast.BlockStatement], env: environment.Environment) -> monkey_object.MonkeyObject:
    ...
    def _eval_block_statement(self, stmts: List[ast.Statement], env: environment.Environment) -> monkey_object.MonkeyObject:
        result: monkey_object.MonkeyObject = Evaluator.null
        for stmt in stmts:
            result = self.eval(stmt, env)
    ...
    def _apply_function(self, function: monkey_object.MonkeyObject,
                        args: List[monkey_object.MonkeyObject]) -> monkey_object.MonkeyObject:
    ...
    def _eval_string_infix_expression(self, operator: str, left: monkey_object.MonkeyObject,
                                       right: monkey_object.MonkeyObject) -> \
                                       monkey_object.MonkeyObject:
        ...
        right_val = right.value
        if operator != "+":
            return monkey_object.Error(f"Unknown operator: {left.type_} {operator} {right.type_}")
